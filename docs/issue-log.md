...
- 已确认并固定：MCP 响应一律用 JSON（`src/mcp/http.ts` 的 `enableJsonResponse: true`），
  不改流式。判据来自工具实际形态：9 个工具全部短同步、只读、一次性结构化输出，
  大集合用 `limit/offset` 分页，`src/mcp` 内没有 progress/logging 通知、sampling
  或等待（无中间状态可推）。
- 流式能带来的收益里，**唯一真实的一条是抗客户端超时**（首事件/心跳证明连接活着）；
  代价则是削弱无状态设计（真流式要跨请求推送就得引 session，正是该 handler 要避开的）、
  隧道对长响应的 idle/总时长限制与缓冲、失败语义变差（中途断不知消费到哪）、
  以及排障变难（公网瞬断时短请求可重试，长流断了不好续）。
- 将来改为流式的触发条件：出现可能跑数秒以上的工具、需要「边搜边给前 N 条」，
  或需要服务端主动推进度/日志。届时**先只翻 `enableJsonResponse: false`**
  （同一 POST 用 SSE 回，仍无 session）；只有确实需要服务端主动推送才考虑有状态模式
  （session + `GET /mcp` 流），那会在公网端点上重新引入会话状态。
- 理由与触发条件已写进代码注释与 `architecture.md`，避免以后被当成随手可调的开关。
  客户端仍须同时带 `Accept: application/json, text/event-stream`（缺一个常见 406），
  那是请求头要求，不代表服务端必须流式。
